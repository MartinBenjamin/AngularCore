...
describe(
    ...
    {
        describe(
            'Given an Ontology o1 with axioms Class(c1), NamedIndividual(i1), DataProperty(dp1) and DataPropertyAssertion(dp1, i1, 1): ',
            () =>
            {
                ...
                describe(
                    'Given additional axiom HasKey(DataMinCardinality(dp1, 1), [ dp1 ]): ',
                    () =>
                    {
                        ...
                        assert('evaluator.AreEqual(i1, { dp1: [ 1 ] })');
                        assert('!evaluator.AreEqual(i1, { dp1: [ 1, 2 ] })');
                        assert('evaluator.AreEqual({ dp1: 1 }, { dp1: 1 })');
                        assert('evaluator.AreEqual({ dp1: 1 }, { dp1: [ 1 ] })');
                        assert('evaluator.AreEqual({ dp1: [ 1 ]}, { dp1: 1 })');
                        assert('evaluator.AreEqual({ dp1: [ 1 ]}, { dp1: [ 1 ]})');
                        assert('!evaluator.AreEqual({ dp1: 1 }, { dp1: 2 })');
                        assert('!evaluator.AreEqual({ dp1: 1 }, { dp1: [ 1, 2 ] })');
                        assert('evaluator.AreEqual({ dp1: [ 1, 2 ] }, { dp1: [ 1, 2 ] })');
                        assert('evaluator.AreEqual({ dp1: [ 2, 1 ] }, { dp1: [ 1, 2 ] })');
                        assert('evaluator.AreEqual({ dp1: [ 2, 1 ] }, { dp1: [ 2, 1 ] })');
                        assert('evaluator.AreEqual({ dp1: [ 1, 1, 2, 2 ] }, { dp1: [ 1, 2 ] })');

                        describe(
                            'Given additional axiom DataPropertyAssertion(dp1, i1, 2):',
                            () =>
                            {
                                ...
                                assert('!evaluator.AreEqual(i1, { dp1: [ 1 ] })');
                                assert('evaluator.AreEqual(i1, { dp1: [ 1, 2 ] })');
                                assert('evaluator.AreEqual(i1, { dp1: [ 2, 1 ] })');
                            });
                    });
        ...
        describe(
            'Given an Ontology o1 with axioms Class(c1), DataProperty(dp1), FunctionalDataProperty(dp1) and HasKey(DataMinCardinality(dp1, 1), [ dp1 ]): ',
            () =>
            {
                let o1 = new Ontology('o1');
